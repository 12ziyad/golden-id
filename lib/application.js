...
function createWorkflow(options = {}) {
  ...
  const workflow = {
    ...
    /**
     * Compare the documents of ONE application.
     *
     * `documentIds` are optional and are always intersected with what the
     * application owns. An id belonging to another application is not merely
     * ignored — it is an integrity failure that blocks issuance outright.
     */
    async compare({ applicationId, userId, documentIds }) {
      const application = store.getApplication(applicationId, userId);
      if (!application) return { error: 'application_not_found' };

      const owned = store.listDocuments(applicationId);
      const integrity = { ok: true, failures: [] };

      let selected = owned;
      if (Array.isArray(documentIds) && documentIds.length) {
        const resolved = store.resolveOwnedDocuments(documentIds, applicationId);
        selected = resolved.owned;
      ...
      // Group front/back pages within this application only.
      store.clearLogicalDocuments(applicationId);
      const groups = groupPages(usable);
      for (const group of groups) {
        store.createLogicalDocument({
    ...
    issue({ applicationId, userId, comparison, actor }) {
      ...
      const documents = store.listDocuments(applicationId).filter(document => document.status === 'ready');
