  ...
  const workflow = {
    ...
    async ingest({ applicationId, userId, files, source = 'files' }) {
      ...
      for (const file of discovered.files) {
        ...
        // The extractor has been upgraded since this row was read: re-uploading
        // the same file re-reads it in place with the current pipeline instead
        // of pointlessly answering "duplicate" from a stale extraction.
        if (existingDoc.extractionKey
          && extractor.extractionKeyFor
          && existingDoc.extractionKey !== extractor.extractionKeyFor(file.buffer)) {
          retries.push({ file, document: existingDoc });
          continue;
        }
    ...
    presentDocument(document) {
      ...
      return redactNumbers({
        ...
        // The batch a document arrived in, and the hash of its bytes — the
        // client links uploads to its staged files by CONTENT, never by
        // position across the whole application.
