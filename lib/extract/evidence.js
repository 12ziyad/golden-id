...
function assessField({ field, value, evidenceText, pageText, source, crossConfirmed = false, formatValidated = false, required = false }) {
  ...
  if (inEvidence && hasLabel) {
    ...
    // The veto also needs enough text to be worth trusting: a few characters of
    // failed OCR is not evidence of absence.
    const corroborationUsable = pageText && String(pageText).replace(/\s+/g, '').length >= 80;
  ...
  if (!hasLabel) {
    // No label for this field anywhere on the page. This is the Voter-ID-DOB
    // case: the card carries no date-of-birth label, so a returned date cannot
...
module.exports = {
  ...
  EXCLUSION_REASON, assessField, participates, evidenceContainsValue, labelPresent, squash
};
