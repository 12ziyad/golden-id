...
// 3.2.0: garbage local OCR can no longer witness a field's absence, and the
// targeted single-field re-read runs on every quality tier — real phone
// photos of guilloche-background cards were losing correctly read DOBs.
const EXTRACTOR_VERSION = '3.2.0';
...
function createExtractor(options = {}) {
  ...
  async function extractOne({ buffer, mimeType, fileName, hint, pageRole = 'front', country }) {
    ...
    // 11c. TARGETED RE-READ. For identity-critical fields still unevidenced
    //      — on ANY quality tier; real photos routinely defeat local OCR
    //      even when the image itself is fine — ask the vision model to read
    //      that ONE field again, in isolation, from the best available view.
    //      Agreement between the two independent readings verifies (the
    //      single-field prompt instructs "return null, do not guess", which
    //      is what stops a hallucination from simply repeating itself);
    //      disagreement stays uncertain. A field whose label is visibly on
    //      the page but whose value never became readable is marked so,
    //      honestly.
    if (!isText && isImage && vision.configured) {
      const targets = Object.entries(merged.fields)
        .filter(([name, field]) => field && field.raw_value
          && field.status === STATUS.PRESENT_UNCERTAIN
          && ['holder_name', 'dob', 'gender', 'document_number'].includes(name))
        .slice(0, 3);
    ...
    // holder sees.
    if (tier === 'marginal' || recovered.length) {
      quality.enhancement = {
