...
/**
 * Salvage near-JSON that small vision models actually produce: prose around
 * the object, curly quotes, trailing commas, unquoted property names. Repair
 * only ever changes SYNTAX — the values either parse or the read fails; no
 * repair invents content.
 */
function repairModelJson(text) {
  ...
  let candidate = text.slice(start, end + 1)
    ...
    .replace(/([{,]\s*)([A-Za-z_][A-Za-z0-9_]*)(\s*:)/g, '$1"$2"$3'); // bare keys
  try {
...
function parseModelJson(text) {
  ...
  try {
    ...
    const repaired = repairModelJson(cleaned);
