...
/**
 * Build the fingerprint straight from a consensus record. Only VERIFIED
 * values count: a typed, unverified value must not be able to steer which
 * existing Golden ID an applicant collides with (or dodges).
 */
const verifiedValue = field => (field && field.verificationStatus !== 'unverified' ? field.value : '');
const dedupHashForRecord = record => dedupHash({
  name: verifiedValue(record.fields.holder_name),
  dob: verifiedValue(record.fields.dob),
  gender: verifiedValue(record.fields.gender)
});
