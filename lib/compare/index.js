  ...
  for (const document of documents) {
    ...
    for (const member of participating) {
      ...
      if (candidate == null || String(candidate).trim() === '') {
        // The extraction READ something here but could not verify it (the
        // plain value is nulled for unverified fields). Keep the state so the
        // verdict can show WHAT was read instead of a bare dash.
        if (!state && memberState && memberState.rawValue != null && !PARTICIPATING_STATES.has(memberState.status)) {
          state = memberState;
          memberUsed = member;
        }
        continue;
...
function compareDocuments(documents, options = {}) {
  ...
  for (const field of order) {
    ...
    const cluster = clusterField(field, entries, { country, confirmations: options.confirmations });
  ...
  // Surface holder-confirmed variations in the reasons, with both values and
  // the moment of confirmation — the verdict never hides that a person, not a
  // document, resolved the spelling.
  for (const field of fields) {
    for (const variant of field.variants || []) {
      if (!variant.holderConfirmed) continue;
      decision.reasons.push({
        code: 'holder_confirmed_variation',
        detail: `${field.label}: the holder confirmed "${field.value}" and "${variant.value}" describe the same person (confirmed ${variant.confirmedAt || 'earlier'}).`
      });
    }
  }
