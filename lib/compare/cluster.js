...
function clusterField(field, entries, options = {}) {
  ...
  if (!present.length) {
    return {
      field, value: '', agreeing: [], dissenting: [], unreadable, abstained,
      confidence: 0, clusters: [],
  ...
  return {
    ...
    status: dissenting.length
      ? 'disagreement'
      : (agreeing.length >= 2 ? 'agreement' : 'single_source'),
    corroboration: agreeing.length
  };
}

module.exports = { clusterField, normalizeField, sameValue, weightFor, SOURCE_WEIGHTS, OCR_VARIANT_DISTANCE };
