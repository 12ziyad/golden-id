...
const SOURCE_WEIGHTS = {
  ...
  deterministic: 0.95,
  // A holder-corrected transcription of a PRINTED value. Deliberately below
  // the deterministic-win threshold: a typed value can never outvote a QR or
  // MRZ read, and can never make a cluster "evidence-strong". Pure
  // holder-supplied values (source 'user') do not participate at all.
  user_correction: 0.5,
  ocr: 0.7,
  vision: 0.6,
...
function clusterField(field, entries, options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (state && state.status && state.status !== STATUS.PRESENT_VERIFIED) {
      // An UNVERIFIED read of a field this document is EXPECTED to carry
      // counts as unreadable — it keeps the field on screen, keeps required-
      // field policy engaged, and carries the observed value forward so the
      // holder can accept it in one step rather than typing blind.
      const expected = state.status === STATUS.UNREADABLE
        || (state.status === STATUS.PRESENT_UNCERTAIN
          && state.rawValue != null
          && matrix.expects(entry.type, field, entry.pageRole || 'front', country));
      const bucket = expected ? unreadable : abstained;
      bucket.push({
        type: entry.type, documentId: entry.documentId,
        reason: state.status,
  ...
  if (!present.length) {
    // A holder-asserted value keeps the field VISIBLE even when no document
    // expected it: the applicant's entry must surface with its "not verified"
    // label rather than silently vanishing from the verdict and the record.
    const holderAsserted = abstained.some(item => item.reason === 'holder_asserted');
    return {
      field, value: '', agreeing: [], agreeingEntries: [], dissenting: [], unreadable, abstained,
      confidence: 0, clusters: [], corroboration: 0,
      status: unreadable.length || holderAsserted ? 'not_extracted' : 'not_applicable'
    };
  }
