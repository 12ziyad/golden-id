...
function decide({ fields = [], integrity = { ok: true, failures: [] }, documents = [], face = null } = {}) {
  ...
  // Fields whose only value came from the applicant's own hands. They unblock
  // the flow where policy allows, but the verdict must say so out loud and
  // can never call itself a full match because of them.
  const holderSupplied = fields
    .filter(field => (field.abstained || []).some(item => item.reason === 'holder_asserted'))
  ...
  for (const field of fields) {
    ...
    if (field.status === 'not_extracted' && REQUIRED_CONSENSUS_FIELDS.includes(field.label)) {
      // A required field no document could show still lets the holder type it
      // in and continue — unverified, labelled, and never called a match.
      if (holderSupplied.includes(field.label)) continue;
  ...
  // A verdict resting partly on applicant-typed values is at best a partial
  // overlap, and says so.
  const decision = (singleSourced.length === 0 && holderSupplied.length === 0)
    ? DECISIONS.VERIFIED_MATCH
  ...
  return {
    ...
    reasons: [
      ...
      ...holderSupplied.map(label => ({
        code: 'holder_supplied_unverified',
        detail: `${label}: supplied by the applicant; not verified from any uploaded document.`
      }))
    ],
    summary: HUMAN_READABLE[decision],
