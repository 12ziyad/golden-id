...
function decide({ fields = [], integrity = { ok: true, failures: [] }, documents = [], face = null } = {}) {
  ...
  for (const field of fields) {
    ...
    // A conflict needs at least TWO documents that each genuinely showed the
    // field. One verified value plus one we could not evidence is not a
    // disagreement — it is one source and one unknown.
    if (field.agreeing.length < 1) continue;

    // A dissent every party agrees is a misread is a confirmation, not a conflict.
    const hard = field.dissenting.filter(item => !item.needsConfirmation && !item.likely_ocr_variant);
  ...
  const nameConfirmable = nameField
    ? nameField.dissenting.filter(item => item.needsConfirmation || item.likely_ocr_variant).length
    : 0;
  const nameSupport = (nameField ? nameField.agreeing.length : 0) + nameConfirmable;
  ...
  const multiSourced = comparable.filter(field => field.agreeing.length >= 2);
  const singleSourced = comparable.filter(field => field.agreeing.length === 1);
