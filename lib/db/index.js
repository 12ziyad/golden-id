...
function open(file = config.db.path) {
  ...
  ensureColumn(database, 'documents', 'removed_at', 'TEXT');
  return database;
}
  ...
  const store = {
    ...
    updateDocument(documentId, applicationId, patch = {}) {
      ...
      run(`UPDATE documents SET extraction_key = ?, doc_type = ?, status = ?, status_reason = ?,
             quality = ?, page_role = ?, removed_at = ?, updated_at = ?
        ...
        // `removedAt` uses !== undefined so an explicit null CLEARS the stamp
        // on reactivation.
        patch.removedAt !== undefined ? patch.removedAt : (owned.removedAt ?? null),
        now(), documentId, applicationId);
      return store.getDocument(documentId, applicationId);
    },

    /**
     * The newest document in this application holding these exact bytes.
     * Active rows win over removed ones, so a re-upload after removal finds
     * the removed row only when no live duplicate exists.
     */
    findDocumentByHash(applicationId, hash) {
      const row = get(
        `SELECT * FROM documents WHERE application_id = ? AND content_hash = ?
         ORDER BY CASE WHEN status = 'removed_by_user' THEN 1 ELSE 0 END, created_at DESC
         LIMIT 1`,
        applicationId, hash);
     ...
     * Ids are de-duplicated defensively: the same id listed twice must never
     * become two entries, or one document could corroborate itself.
     */
    ...
    _hydrateDocument(row) {
      ...
      // the HOLDER asserts, not one we located on the page.
      //
      // Two tiers, and the distinction is load-bearing:
      //  - The document verifiably PRINTED a value and the holder repaired the
      //    transcription: still comparable for this document, but as holder
      //    input it never counts toward document agreement.
      //  - The document never showed the field (or it was unreadable or
      //    uncertain): the typed value is HOLDER-ASSERTED — displayed and
      //    stored, but it abstains from comparison entirely and can never
      //    turn absence into verification.
      const states = { ...(extraction ? extraction.fieldStates : {}) };
      for (const field of Object.keys(corrections)) {
        const printed = states[field] || {};
        const printedStatus = printed.status || 'not_present';
        const corrected = printedStatus === 'present_verified';
        states[field] = {
          status: corrected ? 'present_verified' : 'holder_asserted',
          verified: corrected,
          source: corrected ? 'user_correction' : 'user',
          reason: corrected
            ? 'user_corrected_printed_value'
            : 'user_supplied_value_not_printed_on_document',
          rawValue: corrections[field],
          // What the document itself showed, kept alongside the correction.
          documentStatus: printedStatus,
    ...
    // -- confirmations ------------------------------------------------------
    //
    // INSERT-only by design. A holder confirmation is evidence about a
    // decision that was taken; there is deliberately no update or delete.

    addConfirmation({ applicationId, comparisonId, field, acceptedValue, otherValues, decision, actor }) {
      const confirmationId = id();
      const timestamp = now();
      run(`INSERT INTO confirmations
             (id, application_id, comparison_id, field, accepted_value, other_values, decision, actor, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        confirmationId, applicationId, comparisonId, field, acceptedValue || '',
        toJson(otherValues || []), decision || 'same_person', actor || '', timestamp);
      return {
        id: confirmationId, applicationId, comparisonId, field,
        acceptedValue: acceptedValue || '', otherValues: otherValues || [],
        decision: decision || 'same_person', actor: actor || '', createdAt: timestamp
      };
    },

    confirmationsFor(applicationId) {
      return all('SELECT * FROM confirmations WHERE application_id = ? ORDER BY created_at ASC', applicationId)
        .map(row => ({
          id: row.id, applicationId: row.application_id, comparisonId: row.comparison_id,
          field: row.field, acceptedValue: row.accepted_value,
          otherValues: fromJson(row.other_values, []),
